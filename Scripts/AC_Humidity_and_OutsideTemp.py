...
def load_ac_hourly_power(csv_path: Path) -> pd.DataFrame:
    """AC CSV is a cumulative total_energy_consumption in kWh.

    Convert to hourly average power (W) by taking deltas between consecutive
    samples, then summing the kWh consumed within each hour. Sum of kWh over
    a 1-hour bucket equals the average power in kW for that hour, so we
    multiply by 1000 to get Watts.
    """
    df = pd.read_csv(csv_path)
    df.columns = df.columns.str.strip()
    ...
    # Drop the first row (no delta) and any negative deltas (counter resets).
    df = df.dropna(subset=["delta_kwh"])
    df = df[df["delta_kwh"] >= 0].copy()

    hourly_kwh = (
        df.set_index("last_changed")["delta_kwh"]
        .resample("1h")
        .sum()
    )
    # Sum of kWh in 1h == average kW for that hour. *1000 -> W.
    hourly_power = (hourly_kwh * 1000.0).rename("power_w").reset_index()
    hourly_power = hourly_power[hourly_power["power_w"] > 0].reset_index(drop=True)

    if hourly_power.empty:
        raise ValueError(f"{csv_path.name} produced no hourly power data.")
...
def main() -> None:
    ...
    for path in (ac_csv, humidity_csv, temp_csv, outside_temp_csv):
        if not path.exists():
            raise FileNotFoundError(f"Missing file: {path}")

    power_hourly = load_ac_hourly_power(ac_csv)
    ...
    power_stats_text = format_stats(power_hourly["power_w"], "AC Power hourly avg", " W")
    ...
    ax1.bar(
        ...
        color="tab:blue",
        alpha=0.65,
        label="AC Hourly Power Avg",
    )
    ax1.set_ylabel("AC Average Power per Hour (W)", color="tab:blue")
    ...
    plt.title(
        "Midea AC Hourly Avg Power (bars), Humidity (green), "
        "Indoor Temp (orange) and Outside Temp (red dashed)"
    )
    plt.tight_layout()

    output_file = output_folder / "ac_power_humidity_indoor_outside_temp.png"
