# Περιγραφική Στατιστική

Μετά την οπτικοποίηση των δεδομένων και την ανάλυση τους θέλουμε να υπολογίσουμε τις στατιστικές τιμές και να γίνει οπτικοποίηση των αποτελεσμάτων. 

Σύμφωνα με τα δεδομένα λοιπόν έχουμε:
- το αναγνωριστικό του αισθητήρα (`entity_id`)
- την τιμή μέτρησης (`state`)
- τη χρονική στιγμή καταγραφής (`last_changed`)

## 1. Υπολογισμός Στατιστικών Τιμών

### 2. Αφρυγαντήρας (Dehumidifier)

Καταγραφή για μια εβδομάδα. 
Η μέγιστη τιμή της κατανάλωσης (W) δηλαδή το μεγαλύτερο καταγραμμένο state στο διάστημα αυτό των 7ημερών είναι 803.10
H ελάχιστη τιμή της κατανάλωσης (W) δηλαδή το μικρότερο καταγραμμένο state στο διάστημα αυτό των 7ημερών είναι 

<img src="Min-Max_&_avarage_images/history-Dehulidifier-plug2.png" alt="history-Dehulidifier-plug" width="850">


### 3a. Κλιματιστικό (Midea Living Room AC)


<img src="Min-Max_&_avarage_images/history-Midea-living room-AC.png" alt="history_Midea_living_room" width="850">

### 3b. 1-day zoom

<img src="Min-Max_&_avarage_images/history-Midea-living room-AC-1-day-zoom.png" alt="history_Midea_living_room-AC-1-day-zoom" width="850">
